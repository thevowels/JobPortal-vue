...
const props = defineProps(['experiences', 'categories','job'])
...
const handleSubmit = (e) => {
    e.preventDefault();
    form.post(route('my-jobs.update', props.job), {
        ...
        <template #header>
            <Breadcrumb>
                <BreadcrumbList>
                    ...
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
                        <BreadcrumbLink :href="route('jobs.show', job)">
                            {{job.title}}
                        </BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator />
            ...
            <div class="mx-auto max-w-4xl mt-8">
                <Card class="bg-white rounded-lg border border-slate-300 shadow-md py-7 max-w-2xl mx-auto p-6 ">
                    <form @submit.prevent="handleSubmit" class="flex flex-col gap-2">
                        ...
                        <div class="mt-4">
                            <Button variant="outline" class="w-full bg-slate-100 shadow-md border border-slate-300">Save Changes</Button>
                        </div>
                    </form>
                </Card>
